package cn.xzxy.lewy.graph.structure;
...
public class MatrixUDG {
  ...
  public MatrixUDG() {
    ...
    int vlen = IOUtils.readInt();
    System.out.println("input edge number: ");
    int elen = IOUtils.readInt();
    ...
    for (int i = 0; i < elen; i++) {
      ...
      char c1 = IOUtils.readChar();       // 读取"起始顶点"
      char c2 = IOUtils.readChar();       // 读取"结束顶点"
      int weight = IOUtils.readInt();     // 读取"权值"
  ...
  private MatrixUDG(char[] vexs, int[][] matrix) {
    ...
    // 初始化"顶点"
    mVexs = new char[vlen];
    System.arraycopy(vexs, 0, mVexs, 0, vexs.length);

    // 初始化"边"
    mMatrix = new int[vlen][vlen];
    System.arraycopy(matrix, 0, mMatrix, 0, matrix.length);
  ...
  /*
   * Dijkstra最短路径：统计图中"顶点vs"到其它各个顶点的最短路径。
   *
   * 参数说明：
   * vs -- 起始顶点(start vertex)，即计算"顶点vs"到其它顶点的最短路径。
   * prev -- 前驱顶点数组，即prev[i]的值是"顶点vs"到"顶点i"的最短路径所经历的全部顶点中，
   *         位于"顶点i"之前的那个顶点。
   * dist -- 长度数组，即dist[i]是"顶点vs"到"顶点i"的最短路径的长度。
   */
   ...
   */
  private void floyd(int[][] path, int[][] dist) {
